		...
		<div class="row">
			...
			<div class="col-xl">
				<div class="card" style="margin-top:100px;border-radius: 13px;background-color:skyblue;width:800px;">
					<h3 style="text-align:center;color:black;font-size:26px;" class="pt-3">Add Student</h3>
					...
					?>
					<div class="card-body" >
					<form action="student_form_submit.php" method="post" name="myform" id="formid" class="align-center">	
						<div class="row">
							<div class="col-xl-6">
								<div class="form-group">
										<label for="exampleInputPassword1" style="color:black;font-size:13px;" class="mx-2">Student Name</label>
										<input type="text" class="form-control mt-2 mx-2" id="student_name" placeholder="name" name="student_name" autocomplete=off style="border-radius:7px;width:95%;font-size:16px;">
									</div>
									<div class="form-group mt-3">
										<label for="exampleInputPassword1" style="color:black;font-size:13px;" class="mx-2">Mobile Number</label>
										<input type="number" class="form-control mt-2 mx-2" id="user_nobileNumber" placeholder="mobile number" name="student_number" autocomplete=off style="border-radius:7px;font-size:16px;width:95%;">
							...
							<div class="form-group">
									...
									<input type="email" class="form-control mt-2 mx-2" id="user_email" placeholder="email" name="student_email" autocomplete=off style="border-radius:7px;font-size:16px;width:95%;">
								</div>
								...
								<div class="form-group  mt-3">
									...
									<input type="text" class="form-control mt-2 mx-2" id="user_address" placeholder="address" name="student_address" autocomplete=off style="border-radius:7px;font-size:16px;width:95%;">
								</div>
							</div>
						</div>
						<div class="py-4">
							<button type="submit" class="btn  mt-4 btn-block mx-2" id="submitbtn" style="width:98%;background-color:blue;color:white;">Submit</button>
						</div>
					</form>	
					</div>
				</div>
...
<script>
	$(document).ready(function() {
		...
		$("#submitbtn").click(function(e) {
			...
			let user_nobileNumber = $("#user_nobileNumber").val();
			...
			if ((student_name == null || student_name == "") && (user_nobileNumber == null || user_nobileNumber == "") && (user_email == null || user_email == "") && (user_address == null || user_address == "")) {
				$("#err_alert").show();
				$("#success_alert").hide();
				$("#student_name").css("border", "1px solid #E74C3C");
				$("#user_nobileNumber").css("border", "1px solid #E74C3C");
				$("#user_email").css("border", "1px solid #E74C3C");
				$("#user_address").css("border", "1px solid #E74C3C");
			} else if ((student_name == null || student_name == "") || (user_nobileNumber == null || user_nobileNumber == "") || (user_email == null || user_email == "") || (user_address == null || user_address == "")) {
				$("#err_alert").show();
				$("#success_alert").hide();
				$("#student_name").css("border", "1px solid #E74C3C");
				$("#user_nobileNumber").css("border", "1px solid #E74C3C");
				$("#user_email").css("border", "1px solid #E74C3C");
				$("#user_address").css("border", "1px solid #E74C3C");
			} else if (user_nobileNumber.length < 10 || user_nobileNumber.length > 10) {
				$("#user_nobileNumber").css("border", "1px solid #E74C3C");
				$("#error8").show();
				$("#error5").hide();
				$("#error6").hide();
				...
				let user_email = $("#user_email").val();
				$.ajax({
					url: 'http://localhost:8002/institute-managment/validate_userEmail.php',
					data: {
					...
					success: function(data) {
						if (data == 333) {
							...
							let user_mobileNumber = $("#user_nobileNumber").val();
							$.ajax({
								url: 'http://localhost:8002/institute-managment/validate_usernumber.php',
								data: {
